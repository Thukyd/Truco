def show_envido_info(self):
        """Display information about Envido in Truco"""
        print("\n" + "="*60)
        print(f"{'💡 ENVIDO IN ARGENTINIAN TRUCO 💡':^60}")
        print("="*60)
        ...
        print("\n📝 Note: For simplicity, this learning game doesn't implement Envido betting.")
        print("       Focus on learning the card values and Truco betting first.")
        print("="*60)
...
class TrucoGame:
    ...
    def play_game(self):
        ...
        while not any(team.has_won_game() for team in self.teams):
            ...
            input("\nPress Enter to start playing this hand...")
    ...
    def human_turn(self):
        ...
        if self.round_cards:
            ...
            for i, (p, card) in enumerate(self.round_cards):
                print(f"{p.name}: {card.get_display()} ({self.get_card_strength_description(card)})")
        ...
        while not valid_choice:
            try:
                ...
                if 1 <= choice <= len(player.hand):
                    ...
                    print(f"\n🎯 You played: {card.get_display()} ({self.get_card_strength_description(card)})")
                    valid_choice = True
                else:
    ...
    def ai_turn(self):
        ...
        print(f"\n🤖 {player.name} plays: {card.get_display()} ({self.get_card_strength_description(card)})")
    ...
    def determine_round_winner(self):
        ...
        for player, card in self.round_cards:
            print(f"{player.name}: {card.get_display()} ({self.get_card_strength_description(card)})")
